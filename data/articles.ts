...
export const homepageArticles = gql`
  query HomepageArticles {
    articleCollection(locale: "cs", limit: 3, order: sys_publishedAt_DESC, where: { hide_not: true }) {
      items {
        sys { 
          id
          publishedAt
...
query ListArticles ($limit: Int!, $skip: Int!, $category: String!) {
  articleCollection(locale: "cs", order: sys_publishedAt_DESC, limit: $limit, skip: $skip, where: { type_contains: $category, hide_not: true }) {
      items {
        sys { 
          id
          publishedAt
